document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchData() {
        const { data: m } = await supabase.from('singles_matches').select('*');
        const { data: p } = await supabase.from('singles_teams').select('*');
        matches = m || [];
        players = p || [];
        populateTeamFilter();
    ...
    function renderMatchesToScore() {
        ...
        incompleteMatches.forEach(match => {
            const team1 = players.find(p => p.id === match.team1_id);
            const team2 = players.find(p => p.id === match.team2_id);
    ...
    async function saveMatchScore(matchId, modal) {
        ...
        const { error } = await supabase.from('singles_matches')
            .update({
